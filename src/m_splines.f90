  !*********************************************************************/
  !** This code has been download.
  !** Authors: Ondřej Čertík
  !**   download from: https://github.com/certik/fortran-utils/blob/master/src/splines.f90#L9
  !*********************************************************************/
  !** The original code has been adapted in the Barcelona Center for 
  !** Subsurface  Imaging (BCSI).
  !** Authors of the modification/adaptation : Daniel Dagnino.
  !*********************************************************************/
  !** Short resume of this code/file:
  !** Read below and/or the github web page.
  !*********************************************************************/
  module m_splines
...
!     use lapack, only: dgesv, dgbsv
    ...
    function spline3(x, y, xnew) result(ynew)
      ...
      ! interpolating cubic.
      real(8), intent(in) :: x(:), y(:), xnew(:)
      real(8) :: ynew(size(xnew))
      real(8) :: c(0:4, size(x)-1)
      integer :: i, ip
      ! get spline parameters: 2nd derivs at ends determined by cubic interpolation
      call spline3pars(x, y, [2, 2], [0.d0, 0.d0], c)
    ...
    subroutine spline3ders(x, y, xnew, ynew, dynew, d2ynew)
      ...
      real(8), intent(in) :: x(:), y(:), xnew(:)
      real(8), intent(out), optional :: ynew(:), dynew(:), d2ynew(:)
      real(8) :: c(0:4, size(x)-1)
      integer :: i, ip
      call spline3pars(x, y, [2, 2], [0.d0, 0.d0], c)
    ...
    subroutine spline3pars(xi,yi,bctype,bcval,c)
      ...
      real(8), intent(in):: xi(:)        ! x values of data
      real(8), intent(in):: yi(:)        ! y values of data
      ...
      real(8), intent(in):: bcval(2)     ! boundary condition values at each end:
        ! bcval(1) = value at left end, bcval(2) = value at right end
      real(8), intent(out):: c(0:,:)     ! parameters defining spline: c(i,j) = ith parameter of jth
      
        ! spline polynomial, p_j = sum_{i=1}^4 c(i,j) (x-c(0,j))^(i-1), j = 1..n-1, n = # of data pts.
        ! dimensions: c(0:4,1:n-1)
      real(8) As(5,2*size(c,2))             ! spline eq. matrix -- LAPACK band form
      real(8) bs(2*size(c,2))               ! spline eq. rhs vector
      real(8) cs(2*size(c,2))               ! spline eq. solution vector
      real(8) hi(size(c,2))                 ! spline intervals
      real(8) Ae(4,4)                       ! end-cubic eq. matrix
      real(8) be(4)                         ! end-cubic eq. rhs vector
      real(8) ce(4)                         ! end-cubic eq. solution vector
      real(8) xe(4),ye(4)                   ! x,y values at ends
      real(8) d2p1,d2pn                     ! 2nd derivatives at ends
      real(8) x0                            ! expansion center
      real(8) c1,c2,c3,c4                   ! expansion coefficients
      integer n                              ! number of data points
      integer i,j,i2
      ! lapack variables
      integer ipiv(4),ipiv2(2*size(c,2))
      real(8) bemat(4,1),bmat(2*size(c,2),1)
      integer info
      ...
      if (bctype(1) < 1 .or. bctype(1) > 2) stop "spline3pars error: bctype /= 1 or 2."
      if (bctype(2) < 1 .or. bctype(2) > 2) stop "spline3pars error: bctype /= 1 or 2."
      if (size(c,1) /= 5) stop "spline3pars error: size(c,1) /= 5."
      if (size(c,2) /= size(xi)-1) stop "spline3pars error: size(c,2) /= size(xi)-1."
      if (size(xi) /= size(yi)) stop "spline3pars error: size(xi) /= size(yi)"
      ...
      ! compute interpolating-cubic 2nd derivs at ends, if required
        ! left end
      if(bctype(1)==2) then
        if (n < 4) stop "spline3pars error: n < 4"
        xe=xi(1:4)
        ye=yi(1:4)
      ...
      ipiv = 0
#endif
        if (info /= 0) stop "spline3pars error: dgesv error."
        ce=bemat(:,1)
        d2p1=2*ce(3)
      end if
        ! right end
      ...
      ipiv2 = 0
#endif
      if (info /= 0) stop "spline3pars error: dgbsv error."
      cs=bmat(:,1)
      !write(*,*) cs(1:6)
    ...
    subroutine spline3valder(x,xi,c,val,der)
      ...
      real(8), intent(in):: x            ! point at which to evaluate spline
      real(8), intent(in):: xi(:)        ! spline knots (x values of data)
      real(8), intent(in):: c(0:,:)      ! spline parameters: c(i,j) = ith parameter of jth
        ! spline polynomial, p_j = sum_{i=1}^4 c(i,j) (x-c(0,j))^(i-1), j = 1..n-1, n = # of data pts.
        ! dimensions: c(0:4,1:n-1)
      real(8), intent(out):: val         ! value of spline at x
      real(8), intent(out):: der         ! 1st derivative of spline at x
      integer n                           ! number of knots
      integer i1

      ! initialize, check input parameters
      n=size(xi)
      if (size(c,1) /= 5) stop "spline3 error: size(c,1) /= 5."
    ...
    integer function iix(x, xi) result(i1)
      ...
      ! often the case)
      real(8), intent(in) :: x            ! target value
      real(8), intent(in) :: xi(:)        ! mesh, xi(i) < xi(i+1)
    ...
    function iixun(x,n,x1,xn)
      ...
      real(8), intent(in):: x1           ! initial point of mesh
      real(8), intent(in):: xn           ! final point of mesh
      integer i
    ...
    function iixexp(x,n,x1,alpha,beta)
      ...
      integer, intent(in):: n             ! number of mesh points
      real(8), intent(in):: x1           ! initial point of mesh
      real(8), intent(in):: alpha        ! mesh parameter:
      !   x(i) = x1 + alpha [ exp(beta(i-1)) - 1 ], i = 1 .. n,
      ! where alpha = (x(n) - x(1))/[ exp(beta(n-1)) - 1 ],
      ! beta = log(r)/(n-2), r = (x(n)-x(n-1))/(x(2)-x(1)) = ratio of last to first interval,
      real(8), intent(in):: beta         ! mesh parameter
      integer i
    ...
    function poly3(x,c)
      ...
      real(8) poly3
      real(8), intent(in):: x      ! point at which to evaluate polynomial
      real(8), intent(in):: c(0:)  ! coefficients: poly = \sum_{i=1}^4 c(i) (x-c(0))^(i-1)
      real(8) dx
      dx=x-c(0)
